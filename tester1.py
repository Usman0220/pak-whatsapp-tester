...
# --- Config ---
TARGET_REGISTERED = 5   # Stop after finding this many registered numbers
HTTP_TIMEOUT = 5
NEGATIVE_REGEX = re.compile(r"Chat on WhatsApp", re.IGNORECASE)
MAX_CONCURRENT_REQUESTS = 20  # Number of parallel requests per batch
BATCH_SIZE = 10  # Number of numbers generated per batch
        ...
        while len(registered) < TARGET_REGISTERED:
            ...
            for n in results:
                if n is not None and len(registered) < TARGET_REGISTERED:
                    registered.append(n)
                    print(f"{len(registered):02}. Local: {n['local']} | waLink: {n['waLink']} -> ✅ Registered")
